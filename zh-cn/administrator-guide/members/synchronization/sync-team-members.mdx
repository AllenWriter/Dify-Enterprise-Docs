...
配置完身份供应商后，前往管理后台，点击**成员** → **右侧菜单按钮** → **自动同步** → **启用**按钮。

![](https://assets-docs.dify.ai/2025/05/08d3b05fc5dfaa1eb0a6b8bd523ea5f9.png)

> 开启后，自动同步并非实时进行。同步时间取决于身份供应商内应用的自动执行日期，你可以前往身份供应商查看自动同步历史记录。

## 重要提示

<Info>
  启用 SCIM 同步后，为确保数据一致性，身份提供商 (IdP) 将成为用户和群组管理的唯一权威来源。企业级 APIs 的部分功能将受到限制。
</Info>

...
<Frame caption="AWS CDK アーキテクチャ図">
  <img src="/ja-jp/ja-jp/img/aws-cdk-1.png" alt="AWS CDK アーキテクチャ図" />
</Frame>

### テストデプロイ

| **コンポーネント**  | **Helm Chart 設定** | **数量** | **vCPU** | **メモリ (GB)** | **ストレージ (GB)** | **メモ** |
| ------------------ | -------------------- | -------- | -------- | --------------- | ------------------- | -------- |
| S3                 | persistence          | 1        |          |                 |                     |          |
...
4. ### 環境変数の設定：
   ...
   - `AWS_EKS_CHART_REPO_URL`：（AWS 中国リージョンのみ）AWS EKS Helm chart リポジトリ URL。
...
7. ### AWS EKS アクセス権の更新：
   ...
   1. EKS クラスターのパネルに移動し、「アクセス」メニューを選択し、「管理アクセス」をクリックします。
      <Frame caption="EKS クラスターのアクセス管理">
        <img src="/ja-jp/ja-jp/img/aws-cdk-2.png" alt="Dify-Testing-DifyStackTest-EKS" />
      </Frame>
   2. 「管理アクセス」ダイアログで、「EKS API と ConfigMap」を選択し、「変更を保存」をクリックします。
   3. IAM アクセスエントリパネルで、「アクセスエントリを作成」をクリックします。
      <Frame caption="IAMアクセスエントリ">
        <img src="/ja-jp/ja-jp/img/aws-cdk-3.png" alt="IAM access entries" />
      </Frame>
...
14. ### TLS 設定：
    ...
    AWS `ACM` に移動して、Helm chart 設定ファイル `values.yaml` で宣言されたドメイン名の証明書を申請します。

...
<Tip>
   You must choose ARM architecture servers to properly install Dify Enterprise.
</Tip>
...
During the testing phase, you need at least the following environment to launch the most basic Dify Enterprise system:

* **Kubernetes 1.24+ environment**
   * 4 CPU cores, 16 GB memory
* **Helm 3.14+**
* **Kubectl**
* **S3 storage or S3-compatible persistent storage**
   * Minimum capacity of 50 GB (100 GB recommended)

This system has the following characteristics:
* Contains all software functions of Dify Enterprise
* Low availability
* Low data reliability
* Not suitable for any production or production-like environment

You can also use **Minikube** for deployment. Minikube is a lightweight local Kubernetes cluster suitable for early feature trials and learning Dify Enterprise's software capabilities. **Do not use this solution in a production environment**; if server issues occur, your data may not be saved.

At this stage, please ensure you involve your company's operations engineers or relevant server maintenance personnel to obtain sufficient resources and reduce communication costs. If you are unfamiliar with concepts and applications like Kubernetes, Helm Chart, Kubectl, etc., we recommend contacting technical personnel with relevant capabilities for support or contacting the Dify sales team for deep technical support quotes. If you have already purchased corresponding deep technical support services during the contract phase, we will assign a dedicated team to assist you with deployment.
...
* **Persistent storage**
   * S3, Azure Blob, Alibaba Cloud OSS, Google Cloud Storage, etc.
...
* **Kubernetes cluster**
   * 3 Master Nodes: 4 CPU cores, 16 GB memory each
...
Typically, you need to install the following infrastructure software:
...
After contract signing, you will obtain the **update method** in the enterprise version deployment manual. You can choose not to update Dify Enterprise, but your **access key** will only be effective during the contract-specified time.
...
1. **Dify Enterprise account and password**—Used to generate tokens for accessing Dify Enterprise images.

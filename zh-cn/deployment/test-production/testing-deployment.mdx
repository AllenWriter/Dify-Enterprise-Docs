...
- Redis 数据库
    - 版本 6+
    - 1 GB RAM
- [向量数据库虚拟机 - Qdrant 单节点](/zh-cn/deployment/advanced-configuration/qdrant/single-node)
    - 1 台虚拟机
    - 4 CPU，8 GB RAM
    - 100 GB 存储空间
...
### 安装 Dify Enterprise
你可以使用以下指南安装 Dify Enterprise。但是你需要使用 `values-no-infra-for-testing.yaml` 而不是 `values.yaml`。

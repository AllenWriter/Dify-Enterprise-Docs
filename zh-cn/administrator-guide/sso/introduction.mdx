...
* 启用后，用户访问任何由 Dify 企业版平台创建的 Web 应用之前都需要通过身份提供商（IdP）的身份验证，鉴权后才能够被允许访问。该设置为全局选项，暂时无法单独控制某个单独的 WebApp 开启/关闭权限。如需开启对单独 Webapp 进行人员权限分组，请参考[应用访问权限管理](/versions/2-8-x/zh-cn/user-guide/application-publishing/permission-management)。

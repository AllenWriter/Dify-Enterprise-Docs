...
2. 按照页面提示填写应用名称、按照需求定义授权范围等信息，其中 `Sign-in redirect URIs` 字段需填写 Dify 企业版 Callback URL，获取方法请参考[下文](#2-callback-url)。
...
2. 将其粘贴至 Okta OAuth2 App 内的 **Sign-in redirect URIs** 字段内。
...
2. 切换至 "Sign On" 页中找到 Issuer 字段，将 **Issuer URL** 切换为固定链接。
